import { pgTable, uuid, varchar, text, boolean, integer, timestamp, date, time, pgEnum, jsonb, uniqueIndex, index } from 'drizzle-orm/pg-core';

// Enums
export const userRoleEnum = pgEnum('user_role', ['admin', 'organizer']);
export const eventTypeEnum = pgEnum('event_type', ['birthday', 'xv', 'baptism', 'wedding', 'graduation']);
...
export const templates = pgTable('templates', {
  ...
  sectionsSchema: jsonb('sections_schema').$type<Record<string, unknown>[]>().default([]),
...
export const events = pgTable('events', {
  ...
  customConfig: jsonb('custom_config').$type<Record<string, unknown>>().default({}),
...
export const eventSections = pgTable('event_sections', {
  ...
  index('idx_sections_event_order').on(table.eventId, table.order),
]);
...
export const eventMedia = pgTable('event_media', {
  ...
  sectionId: uuid('section_id').references(() => eventSections.id, { onDelete: 'set null' }),
  metadata: jsonb('metadata').$type<Record<string, unknown>>().default({}),
  createdAt: timestamp('created_at').defaultNow().notNull(),
}, (table) => [
  index('idx_media_event').on(table.eventId),
]);
...
export const eventViews = pgTable('event_views', {
  ...
  index('idx_views_event_date').on(table.eventId, table.viewedAt),
]);
